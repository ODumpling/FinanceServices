...
export function CreateMembershipForm({
  ...
  function addMemberToFund(user: IMemberDto) {
    ...
    financeApi().then((client) =>
      client.memberships_CreateMembership(command).then(() => {
        hasSubmit("true");
      })
    );
  }
  ...
  function removeMemberToFund(user: IMemberDto) {
    ...
    financeApi().then((client) =>
      client.memberships_DeleteMembership(command).then(() => {
        hasSubmit("true");
      })
    );
  }
  ...
  return (
    <Fragment>
      {!addUserView ? (
        <CurrentUsersView
          people={members}
          newUserClicked={(data) => setAddUserView(data)}
          removeUserEvent={(data) => removeMemberToFund(data)}
        />
      ) : (
        <AddUsersView
          users={nonMembers}
          backButtonClicked={() => setAddUserView(false)}
